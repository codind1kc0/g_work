...
loader_target:
    ...
    mov cx, root_dir_len
	mov bx, [bp + 10];mov bx, buf
	call read_sector	

    mov si, [bp + 2];mov si, target_str
	mov cx, [bp + 4];mov cx, 11
    call found_root_dir
    ...
    ;mov bp, sp ;protect bp 
    ; copy fat1 table to 0x9000
    mov ax, fat_entry_len
	mov bx, [BPB_BytsPerSec]
	mul bx ; 512*9
    mov bx, [bp + 6];mov bx, base_of_target
	sub bx, ax
...
; to small range: only 0x0000 - 0xffff 
    mov es, [bp + 8];mov si, base_of_target / 0x10
	                ; mov es, si
	mov si, 0
